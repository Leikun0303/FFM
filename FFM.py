...
def parse_args(check=True):
    ...
    parser.add_argument('--nprint', type=int, default=50)
    parser.add_argument('--batch_size', type=int, default=512)
    parser.add_argument('--reg_L1', type=float, default=2e-3)
...
def train_model(sess, model, epochs=10, print_every=500):
    ...
    for e in range(epochs):
        ...
        train_data = pd.read_csv(input_dir+'/train_sample200w.csv', chunksize=model.batch_size)
        # batch_size data
...
def test_model(sess, model, print_every = 50):
    ...
    test_data = pd.read_csv(input_dir+'/test_sample10w.csv',
    ...
    for data in test_data:
        ...
        if test_step == 1:
            data[['id','click']].to_csv(output_dir+'/FFM_pred.csv', mode='a', index=False, header=True)
        else:
            data[['id','click']].to_csv(output_dir+'/FFM_pred.csv', mode='a', index=False, header=False)
...
if __name__ == '__main__':
    ...
    for field in fields:
        azip = zipfile.ZipFile(input_dir+'/dicts.zip')
        fields_dict[field] = pickle.load(azip.open(field+'.pkl'))
        azip.close()
        # with open(train_path+'/dicts/'+field+'.pkl','rb') as f:
        #     fields_dict[field] = pickle.load(f)

    # loading feature2field dict
    azip = zipfile.ZipFile(input_dir+'/features.zip')
    feature2field = pickle.load(azip.open('feature2field.pkl'))
    azip.close()
    # with open(train_path+'/features/feature2field.pkl','rb') as f:
    ...
    config['reg_l1'] = FLAGS.reg_L1
    config['reg_l2'] = FLAGS.reg_L2
    config['k'] = FLAGS.k
    ...
    with tf.Session() as sess:
        ...
        train_model(sess, model, epochs=Nepochs, print_every=Nprint)
        # print('start validation...')
        # validation_model(sess, model, print_every=100)
        print('start testing...')
        test_model(sess, model, print_every=Nprint)